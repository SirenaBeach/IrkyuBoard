<?php
die;
	// block of custom sidebars that used to be on postcode()
	// use them as reference for creating new ones, I guess :|
	...
	$set[tdbg] rowspan=2 style='padding: 5px 1px 5px 1px;'>
	  <center>$set[userlink]<font class='fonts'><br>
	  $set[userrank]
		$syndrome
		<br>
		<br>$set[userpic]
  ...
  if ($post['uid'] == "1" && !$x_hacks['host']) {
		...
		return "<table width=100% cellpadding=0 cellspacing=0 style=\"background: #004c5a; background-position: top right; background-repeat: repeat-x; border: 1px solid #000;\">
			...
			<td valign='top' id=\"post". $post['id'] ."\">$post[headtext]$post[text]$post[signtext]</td></table>
		";
		...
		return "<table class='table'>
			...
			$set[tdbg] style=\"padding: 0;\" id=\"post". $post['id'] ."\">$post[headtext]$post[text]$post[signtext]</td></table>
		";
...
// ************************************************************
		...
		$set[tdbg] rowspan=2 style='padding: 5px;'>
		  <img src='images/smilies/bigeyes.gif' title='o_O' align='absmiddle'> $set[userlink]<font class='fonts'><br>
		  ". ($set['userrank'] ? "<br>". $set['userrank'] : "") ."
				$syndrome
				<br>
				$set[userpic]
...
*/
	...
	elseif ($post['uid'] == 1090 && !$x_hacks['host']) {
		...
		$brsidebar = "
			...
			$set[tdbg] rowspan=2>
				...
				(!empty($syndrome) ? "<div class=\"brsyndrome\">$syndrome</div>" : "") . "
  ...
  // (Adelheid uses this)
	...
	<table class='table'>
	$set[tdbg] rowspan=2>
	  $set[userlink]<font class='fonts'><br>
	  $set[userrank]$syndrome<br>
	  <br><img src=images/_.gif width=200 height=1>
	</td>
	$set[tdbg] height=1 width=100%>
	  <table cellspacing=0 cellpadding=2 width=100% class=fonts>
