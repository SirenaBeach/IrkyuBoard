<?php
  $config['board-title']="
	Ninja AE is a moron!
	<embed src='schemes/aesucks/css-auth.mid' autostart=true loop=true autoplay=true hidden=true style='display: none'>
	<br>a special gift to you from acmlm and xkeeper
	<script>window.alert(\"The whole point of this great color scheme is to point out that Ninja AE sucks. Enjoy!\");</script>
	<br>YOU ARE DOOMED!! FEAR THE -8388608 ...
  ";
  $schemedir     = "schemes/aesucks/";
  $numdir        = 'num3/';
  $numfil        = 'numnes';
 // $font='MS Serif';
 // $font2='MS Serif';
  $textcolor     = 'FF0000';
  $linkcolor     = 'FF0000';
  $linkcolor2    = 'FF0000';
  $bgimage       = 'images/ranks/mario/boo.gif';
  $bgcolor       = '00FF00';
  $newthreadpic  = 'Ninja AE,';
  $newreplypic   = 'you still suck!';
  $tablebg1      = "0000FF url('{$schemedir}/flash3.gif')";
  $tablebg2      = "00FF00 url('{$schemedir}/flash1.gif')";
  $categorybg    = "FF00FF url('{$schemedir}/flash2.gif')";
  $tableheadtext = "FFFF00";
  $tableheadbg   = "FFFFFF url('{$schemedir}/flash2.gif')";
  $tableborder   = "00FFFF url('{$schemedir}/flash1.gif')";